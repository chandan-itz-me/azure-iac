...
module "application_gateways" {
  ...
  ] : [module.application_gateway_identities[each.key].identity_ids["workload"]]
...
module "api_managements" {
  ...
  identity_ids = try(each.value.identity_type, "UserAssigned") == "UserAssigned" ? (
    length(try(each.value.managed_identity_keys, [])) > 0 ? [for key in each.value.managed_identity_keys : local.managed_identity_ids[key]] : [module.api_management_identities[each.key].identity_ids["workload"]]
  ) : []
  tags = local.common_tags
}
